...
public class Game {
    ...
    public void play()
    {
        ...
        File file = new File("C:\\Users\\Peri\\Documents\\Askiseis Sxolis\\Refugeoly\\RefugeolyExercise-main\\Refugeoly\\src\\refugeoly\\refugeoly-squares.txt");
...
//           }
//           
//           System.out.println("done seting up players ");
//           
//        }while(pass == false);
...
do {
        for (int i = 0; i < res; i++) {
            ...
            players.add(new Refugee(ans, 10000, board1, squares.get(0), 0));
        }
           ...
           for(int i = 1;i<=res; i++)
            {
                ...
                if(res >= 1 && (!(isAlive(players.get(1)))) && (p1dead == false))
                {
                    players.remove(1);
                    p1dead = true;
                }
                if(res >= 2 && !(isAlive(players.get(2))) && (p2dead == false))
                {
                    players.remove(2);
                    p2dead = true;
                }
                if(res >= 3 && !(isAlive(players.get(3))) && (p3dead == false))
                {
                    players.remove(3);
                    p3dead = true;
                }
                if(res >= 4 && !(isAlive(players.get(4))) && (p4dead == false))
                {
                    players.remove(4);
                    p4dead = true;
                }
